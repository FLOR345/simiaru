...
// Servicio de notificaciones por email para SIMIARU usando Brevo
...
// Configurar transporter con Brevo SMTP
const transporter = nodemailer.createTransport({
  host: 'smtp-relay.brevo.com',
  port: 587,
  secure: false,
  auth: {
    user: process.env.BREVO_USER || 'flormelany01@gmail.com',
    pass: process.env.BREVO_API_KEY
  }
});

// Email desde
const EMAIL_FROM = '"SimiAru" <flormelany01@gmail.com>';

// Verificar conexión al iniciar
transporter.verify((error, success) => {
  if (error) {
    console.log('❌ Error configurando email Brevo:', error.message);
  } else {
    console.log('✅ Servicio de email Brevo configurado correctamente');
  }
});
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto;">
        ...
        <div style="padding: 30px; background: white; text-align: center; border: 1px solid #eee; border-radius: 0 0 16px 16px;">
          ...
          <p>Llevas <strong>${usuario.racha_actual} días</strong> consecutivos aprendiendo. ¡No dejes que se rompa!</p>
          <a href="${FRONTEND_URL}/dashboard" style="display: inline-block; background: #f97316; color: white; padding: 15px 40px; border-radius: 30px; text-decoration: none; font-weight: bold; margin: 20px 0;">
